/**
 * DevLynx AI feed server with AI – port 2847
 * Run: node server-with-ai.js  (from feed-server folder)
 * Optional: npm install for dotenv + openai; otherwise uses built-in .env read and https.
 */
...
try {
  ...
  if (portMatch && portMatch[1]) PORT = parseInt(portMatch[1].trim(), 10) || 2847;
} catch (_) {}
if (process.env.PORT) PORT = parseInt(process.env.PORT, 10) || 2847;
if (!Number.isFinite(PORT)) PORT = 2847;
...
const server = http.createServer(async (req, res) => {
  ...
  // Debug: open http://localhost:2847/test-openai in browser to verify OpenAI connection
  if (req.method === 'GET' && url.pathname === '/test-openai') {
    if (!OPENAI_API_KEY) {
  ...
  if (req.method === 'POST' && url.pathname === '/') {
    ...
    switch (type) {
      case 'screenshot': {
        ...
        const filepath = path.join(SCREENSHOTS_DIR, base);
        try {
